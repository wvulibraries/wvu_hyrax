...
class ContentEventJob < EventJob
  ...
  def perform(repo_object, depositor)
    # if depositor is nil, then the object was created by a batch process and we don't need to notify anyone
    #return unless depositor
    depositor = User.find_by_user_key(repo_object.depositor) if depositor.nil?
